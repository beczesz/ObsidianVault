...
```dataviewjs
// === Weight + Trend (interactive, mobile-friendly) ===
// Range buttons + crosshair tooltips + auto y-axis.

// --- CONFIG ---
// Initial chart range in days. Set to null for "All time".
const DEFAULT_RANGE_DAYS = 30;
// Reference date = this note's title if it's a date, else today.
// EMA-14 trend computed on the FULL series (up to refDate), then sliced.
...
// 1. Pull all weight readings from daily notes up to refDate
...
// All-time minimum of the EMA-14 trend (light-red horizontal reference line)
const trendMinAllTime = fullTrend.length ? parseFloat(Math.min(...fullTrend).toFixed(1)) : null;

// Today's trend value (most recent entry in the full series)
const trendToday = fullTrend.length ? fullTrend[fullTrend.length - 1] : null;

// Label for the all-time trend-min line.
// If today's trend equals the all-time low (within 0.05 kg) → celebratory message.
// Otherwise show both values: "Min trend X kg (Today Y kg)".
const trendMinLabel = (() => {
...
// 4. UI scaffolding: button bar + chart container
...
// 5. Render — clears chartWrap and re-draws with the chosen range
...
// 6. Build buttons + initial render
